...
def intsctn(xo,yo,zo,u,v,w,ci,m):
	#Find if l=ray is horizontal
	if isinf(m):
...
class regions(object):
	def __init(self,point_arr_tple,mat):
		self.shape={'bound':{'r':point_arr_tple[0],'z':point_arr_tple[1],'m':mat}}
		self.shape['internal']=[]
		
	def innerbound(self,point_arr_tple,mat):
		self.shape[internal].append({'r':point_arr_tple[0],'z':point_array_tuple[1],'m'=mat})
	...
	def interrogateinternal(self,start_tuple,dir_tuple):
		...
		mats=[]
		L=[0]
		for i in enumerate(self.shape['bound']['r']):
			...
			tt,nsol=intsctn(xo,yo,zo,u,v,w,c,m)
			for ti in tt:
